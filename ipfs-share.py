...
# Upload file to ipfs
def upload_file(path: str, ipfs: str) -> Tuple[str, str]:
    # ipfs subcommand
...
def remote_pin(pinner: RemotePinner, host: str, cid: str):
    if pinner is RemotePinner.Node:
        # curl -X POST "http://ipfs-node:5001/api/v0/pin/add?arg=QmX6kQeqMJ56QYJ7yHYREZ96Z3qkVGK6qoqFRJfSnmz8Jw
        url = urljoin(host, f"/api/v0/pin/add?arg={cid}")
...
def main(path: str, clipboard: bool, gateways: List[str], pin: bool, pinner_type: RemotePinner = RemotePinner.Node, pinner_url: str = ""):
    if (ipfs := which("ipfs")) is None:
        raise Exception("ipfs binary could not be found")

    folder_cid, file_cid = upload(path, ipfs)

    if pin: 
        remote_pin(pinner_type, pinner_url, folder_cid) 

    urls = [urljoin(g, f"ipfs/{file_cid}") for g in gateways]
...
if __name__ == "__main__":
    gateways = [url(x) for x in env_gateways.split()] if (env_gateways := os.environ.get("IPFS_GATEWAYS")) is not None else None
    remote_pinner_url = url(env_remote_pinner) if (env_remote_pinner := os.environ.get("IPFS_REMOTE_PINNER")) is not None else None
    ...
    parser.add_argument("-g", "--gateway",
                        ...
                        default=gateways)
    parser.add_argument("-r", "--remote-pinner-url", type=url, default=remote_pinner_url, required="-p" in sys.argv and remote_pinner_url is None,
    ...
    parser.add_argument("-p", "--pin", action="store_true",
                        help="Pin file/folder to a remote pinner")

    args = parser.parse_args()

    main(args.path, not args.no_clipboard, args.gateway or GATEWAYS, args.pin, args.pinner_type, args.remote_pinner_url)
